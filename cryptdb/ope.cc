...
#include "crypto_cipher_OpeNative.h"
...
JNIEXPORT jstring JNICALL Java_crypto_cipher_OpeNative_nativeEncrypt(JNIEnv *env,
...
JNIEXPORT jstring JNICALL Java_crypto_cipher_OpeNative_nativeDecrypt(JNIEnv *env,
        ...
        string str = buffer.str();
        char* plaintextStr = new char[str.length()+1];
        strcpy(plaintextStr, str.c_str());

        jstring result = (*env).NewStringUTF(plaintextStr);
...
ZZ OPE::encrypt(const ZZ &ptext) {
  ...
  ZZ result = dr.r_lo + aesrand.rand_zz_mod(nrange);
  return result;
}
